...
typedef struct {
  u08_t super;
  u08_t fields;
} nvm_class_hdr_t; //__attribute__((packed)) nvm_class_hdr_t;
...
typedef struct {
  ...
  u08_t max_locals;
  u08_t max_stack;
} nvm_method_hdr_t; //__attribute__((packed)) nvm_method_hdr_t;
...
} nvm_header_t; // __attribute__((packed)) nvm_header_t;
